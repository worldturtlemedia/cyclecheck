...
/**
 * See: https://github.com/airbnb/MvRx/blob/master/mvrx/src/main/kotlin/com/airbnb/mvrx/Async.kt
...
data class Fail<out T>(
    val error: Throwable,
    val data: T? = null
) : Async<T>(complete = true, shouldLoad = true) {
    ...
    override fun hashCode(): Int = Arrays.hashCode(
        arrayOf(error::class, error.message, error.stackTrace[0])
    )
}
